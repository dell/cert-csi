...
var (
	executionLogFile = "e2e_execution"
)
...
const (
	...
	defaultDir     = "/root/reports/"               // defaultDir is provided the report directory
	...
	BinaryFile   = "/root/kubernetes-test-linux-amd64.tar.gz"                   // BinaryFile binary file
)
...
func DownloadBinary(version string) error {
	...
	if resp.StatusCode != http.StatusOK {
		return errors.New("Unable to download tar file with return code :" + strconv.Itoa(resp.StatusCode) + " with error " + err.Error())
	}
	if err != nil {
...
func CheckKubeConfigEnv() string {
	...
	if kubeConfig == "" {
		...
		return filepath.Clean("/root/.kube/config")
	}
	return ""
}
...
func FileExists(filename string) bool {
	...
	log.Infof("File found: %s", filename)
	return err == nil
}
...
func SkipTests(skipFile string) (string, error) {
	...
	if err != nil {
		_ = fmt.Errorf("YAML-Encoded error :%s", err)
		return "", err
	}
	if len(tests.Skip) == 0 {
		return "", nil
...
func BuildE2eCommand(ctx *cli.Context) ([]string, error) {
	...
	if reportDir == "" {
		...
		if FileExists(reportDir) {
			...
			log.Infof("Old reports not found")
		}
	} else {
		if FileExists(reportDir) {
			err := os.Remove(filepath.Clean(reportDir))
...
func ExecuteE2ECommand(args []string, ch chan os.Signal) error {
	...
	log.Error("e2e command arguments are: ", args)
	...
	go func() {
		...
		fmt.Printf("Do you really want to terminate the process ? (Y/n)")
